...
from r2r.dyenv import DynamicBatch
from r2r.augenv import AugBatch
...
def build_dataset(args, rank=0, is_test=False):
    ...
    feat_db = ImageFeaturesDB(args.img_ft_file, args.lm_ft_file, args.image_feat_size)
    ...
    train_env = AugBatch(
        ...
        block_num=args.max_train_edge, use_inpaint=args.use_inpaint
    )
    ...
    for split in val_env_names:
        ...
        val_env = DynamicBatch(
            ...
            block_num=args.max_eval_edge, use_inpaint=args.use_inpaint
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    if args.eval_first:
        ...
        for env_name, env in val_envs.items():
            ...
            if default_gpu:
                ...
                loss_str += "%s\n" % env_name
                for block_num, metrics in score_summary.items():
                    loss_str += 'block %s' % block_num
    ...
    best_vals = [{'val_unseen': {"spl": 0., "sr": 0., "state":""}} for _ in range(args.max_eval_edge+1)]
    if args.dataset == 'r4r':
        best_vals = [{'val_unseen_sampled': {"spl": 0., "sr": 0., "state":""}} for _ in range(args.max_eval_edge+1)]
        ...
        for env_name, env in val_envs.items():
            ...
            if default_gpu:
                ...
                for block_num, metrics in score_summary.items():
                    ...
                    for metric, val in metrics.items():
                        loss_str += ', %s: %.2f' % (metric, val)
                        writer.add_scalar('%s/%s/%s' % (metric, env_name, block_num), val, idx)
                    loss_str += '\n'

                # select model by spl
                for block_num, best_val in enumerate(best_vals):
                    if env_name in best_val:
                        if score_summary[str(block_num)]['spl'] >= best_val[env_name]['spl']:
                            best_val[env_name]['spl'] = score_summary[str(block_num)]['spl']
                            best_val[env_name]['sr'] = score_summary[str(block_num)]['sr']
                            best_val[env_name]['state'] = 'Iter %d %s' % (iter, loss_str)
                            listner.save(idx, os.path.join(args.ckpt_dir, "best_SPL_%s_%s_%s" % (env_name, str(block_num), args.time)))
        ...
        if default_gpu:
            ...
            for block_num, best_val in enumerate(best_vals):
                for env_name in best_val:
                    write_to_record_file(env_name + ' | ' + str(block_num) + '|' + best_val[env_name]['state'], record_file)
...
def main():
    args = parse_args()
    args.time = datetime.datetime.now().strftime("%m-%d-%H-%M-%S")
