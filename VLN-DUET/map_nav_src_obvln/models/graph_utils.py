...
class GraphMap(object):
    ...
    def update_graph(self, ob):
        ...
        # whether current point is a virtual point
        if len(ob['seen_node']) > 0:
            for k, v in self.another_side.items():
                dist1 = calc_position_distance(ob['position'], self.node_positions[k])
                dist2 = calc_position_distance(ob['position'], self.node_positions[v])
                if v != ob['viewpoint'] and dist1 <= 5 and dist2 <= 5 and not self.graph.visited(k):
                    self.connect_virtual(ob['viewpoint'], k)

        for cc in ob['seen_node']:
            self.node_positions[cc['viewpointId']] = cc['position']
            self.graph.add_edge(ob['viewpoint'], cc['viewpointId'], 95959595)
            self.real_name[cc['viewpointId']] = cc['real_viewpointId']
            self.another_side[cc['viewpointId']] = ob['viewpoint']
        
            # whether the cc is already exist in the graph
            for k, v in self.another_side.items():
                dist1 = calc_position_distance(self.node_positions[v], ob['position'])
                dist2 = calc_position_distance(self.node_positions[v], cc['position'])
                if v != ob['viewpoint'] and dist1 <= 5 and dist2 <= 5 and not self.graph.visited(cc['viewpointId']):
                    self.connect_virtual(v, cc['viewpointId'])

        self.graph.update(ob['viewpoint'])

    def connect_virtual(self, real, virtual):
        self.graph.add_edge(real, virtual, 0)
        self.graph._visited.add(virtual)
        # Combine node embeddings
        if real in self.node_embeds and virtual in self.node_embeds:
            combined_embed = self.node_embeds[real][0]+ self.node_embeds[virtual][0]
            combined_count = self.node_embeds[real][1] + self.node_embeds[virtual][1]
            self.node_embeds[real] = [combined_embed, combined_count]
            self.node_embeds[virtual] = self.node_embeds[real]
        elif real in self.node_embeds:
            self.node_embeds[virtual] = self.node_embeds[real]
        elif virtual in self.node_embeds:
            self.node_embeds[real] = self.node_embeds[virtual]
        else:
            print("NO EMBEDDING FOR BOTH VIRTUAL AND REAL NODES")
