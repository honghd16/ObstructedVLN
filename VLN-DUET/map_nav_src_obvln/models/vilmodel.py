...
class TargetXGraphLXRTXLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(
        self, target_feats, visn_feats, visn_attention_mask,
        graph_sprels=None, first=False
    ):      
        visn_att_output = self.visual_attention(visn_feats, target_feats)[0]
...
class TargetSGraphLXRTXLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        # The cross attention layer
        self.visual_attention = BertAttention(config)

    def forward(
        self, target_feats, visn_feats, visn_attention_mask,
        graph_sprels=None, first=False
    ):  
        target_attention_mask = torch.ones((1, 1), dtype=torch.bool, device=visn_attention_mask.device)
        extended_target_mask = extend_neg_masks(target_attention_mask)
        if graph_sprels is not None:
            visn_attention_mask = visn_attention_mask + graph_sprels
            gmap_ids = visn_attention_mask.shape[-1]
            row_mask = extended_target_mask.repeat(1, 1, gmap_ids, 1)
            col_mask = extended_target_mask.repeat(1, 1, 1, gmap_ids+1)
            combined_attention_mask = torch.cat([row_mask, visn_attention_mask], dim=3)
            combined_attention_mask = torch.cat([col_mask, combined_attention_mask], dim=2)
        else:
            combined_attention_mask = torch.cat([extended_target_mask, visn_attention_mask], dim=-1)

        visn_feats = torch.cat([target_feats, visn_feats], dim=1) if first else visn_feats
        
        visn_att_output = self.visual_attention(visn_feats, combined_attention_mask)[0]

        visn_att_output = self.visn_self_att(visn_att_output, combined_attention_mask)[0]
...
class SearchEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.num_x_layers = config.num_x_layers
        self.x_layers = nn.ModuleList(
            [TargetXGraphLXRTXLayer(config) for _ in range(self.num_x_layers)]
        )

    def forward(self, target_embeds, img_embeds, img_masks, graph_sprels=None):
        extended_img_masks = extend_neg_masks(img_masks) # (N, 1(H), 1(L_q), L_v)
        for i, layer_module in enumerate(self.x_layers):
            img_embeds = layer_module(
                target_embeds, img_embeds, extended_img_masks,
                graph_sprels=graph_sprels, first=(i==0)
            )
...
class LocalVPEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.search_encoder = SearchEncoder(config)
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.target_pos_embeddings = nn.Sequential(
            nn.Linear(config.angle_feat_size*2 + 6, config.hidden_size),
            BertLayerNorm(config.hidden_size, eps=1e-12)
        )
    ...
    def forward_navigation_per_step(
        ...
        visited_global_logits = torch.clone(global_logits)  # Create unmasked fused logits here
        visited_global_logits .masked_fill_(gmap_visited_masks.logical_not(), -float('inf'))
        ...
        outs = {
            ...
            'visited_global_logits': visited_global_logits,
            'obj_logits': obj_logits,
        }
        return outs

    def forward_search_per_step(
        self, target_img_embeds, target_pos_fts, gmap_img_embeds, gmap_step_ids, gmap_pos_fts, 
        gmap_masks, gmap_pair_dists, gmap_visited_masks, gmap_vpids,
        vp_img_embeds, vp_pos_fts, vp_masks, vp_nav_masks, vp_obj_masks, vp_cand_vpids,
    ):
        target_embeds = target_img_embeds + self.target_pos_embeddings(target_pos_fts)
        ...
        gmap_embeds = self.global_encoder.search_encoder(
            target_embeds, gmap_embeds, gmap_masks,
            graph_sprels=graph_sprels
        )

        # local branch
        vp_embeds = vp_img_embeds + self.local_encoder.vp_pos_embeddings(vp_pos_fts)
        vp_embeds = self.local_encoder.search_encoder(target_embeds, vp_embeds, vp_masks)
        ...
        visited_nodes = set([vp for vp, mask in zip(gmap_vpids, gmap_visited_masks[0]) if mask])
        tmp = {}
        bw_logits = 0
        for j, cand_vpid in enumerate(vp_cand_vpids):
            if j > 0:
                if cand_vpid in visited_nodes:
                    bw_logits += local_logits[0, j]
                else:
                    tmp[cand_vpid] = local_logits[0, j]
        for j, vp in enumerate(gmap_vpids):
            if j > 0 and vp not in visited_nodes:
                if vp in tmp:
                    fused_logits[0, j] += tmp[vp]
                else:
                    fused_logits[0, j] += bw_logits
        # print('fused', torch.softmax(fused_logits, 1)[0], fused_logits[0])
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            ...
            return self.forward_search_per_step(
                batch['target_img_embeds'], batch['target_pos_fts'], batch['gmap_img_embeds'], 
                batch['gmap_step_ids'], batch['gmap_pos_fts'], batch['gmap_masks'],
                batch['gmap_pair_dists'], batch['gmap_visited_masks'], batch['gmap_vpids'], 
                batch['vp_img_embeds'], batch['vp_pos_fts'], batch['vp_masks'],
                batch['vp_nav_masks'], batch['vp_obj_masks'], batch['vp_cand_vpids'],
