...
class ImageFeaturesDB(object):
    def __init__(self, img_ft_file, lm_ft_file, image_feat_size):
        self.image_feat_size = image_feat_size
        self.img_ft_file = img_ft_file
        self.lm_ft_file = lm_ft_file
        self._feature_store = {}
        self._block_feature_store = {}
    ...
    def get_block_feature(self, scan, path, edge_0, edge_1):
        key = '%s_%s_%s_%s' % (scan, path, edge_0, edge_1)
        if key in self._block_feature_store:
            ft = self._block_feature_store[key]
        else:
            with h5py.File(self.lm_ft_file, 'r') as f:
                # if key not in f:
                #     reverse_key = '%s_%s_%s_%s' % (scan, path, edge_1, edge_0)
                #     assert reverse_key in f
                #     with h5py.File(self.img_ft_file, 'r') as f:
                #         ft = f[f"{scan}_{edge_0}"][...][:, :self.image_feat_size].astype(np.float32)
                #         self._block_feature_store[key] = ft
                # else:
                ft = f[key][...][:, :self.image_feat_size].astype(np.float32)
                self._block_feature_store[key] = ft
        return ft
